...
public class NetworkManager : MonoBehaviour
{
    ...
    async void SendPingPakcet(byte[] data)
    {
        // 헤더 생성
    ...
    void HandleNormalPacket(byte[] packetData) {
        ...
        if (response.data != null && response.data.Length > 0) {
            ...
            /*if (response.handlerId == 0) {
                var specificData = Packets.Deserialize<InitialPayload>(response.data);
                GameManager.instance.GameStart(specificData.x, specificData.y);
            }*/
    ...
    void ProcessResponseData(byte[] data) {
        try {
            ...
            Debug.Log($"Processed jsonString: {jsonString}");
        } catch (Exception e) {
    ...
    {
        SendPingPakcet(data);
        //GameManager.instance.latency = data;
    }
}
